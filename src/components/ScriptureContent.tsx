...
export const ScriptureContent = () => {
  const { selectedBible, selectedBook, selectedChapter, setError } = useBible();
  ...
  useEffect(() => {
    const fetchScripture = async () => {
      if (!selectedBible || !selectedBook || !selectedChapter) return;

      try {
        setIsLoading(true);
        const data = await bibleApi.getChapter(selectedBible.id, selectedChapter.id);
        setContent(data.content);
      } catch (err) {
        console.error('Error fetching chapter content:', err);
        setError('Failed to load scripture content. Please try again.');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    fetchScripture();
  }, [selectedBible, selectedBook, selectedChapter, setError]);
